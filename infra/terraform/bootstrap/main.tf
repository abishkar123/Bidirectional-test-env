terraform {
  ...
  # per tenant (each tenant has its own subscription and its own bootstrap
  # state — never share this state across tenants).
}
...
locals {
  app_display_name = "sp-${var.tenant_name}-github-deploy"
}
